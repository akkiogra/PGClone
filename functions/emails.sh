...
ls -la /opt/appdata/pgclone/.blitzkeys | awk '{print $9}' | tail -n +4 > /var/plexguide/.emailbuildlist
while read p; do
  cat /opt/appdata/pgclone/.blitzkeys/$p | grep client_email | awk '{print $2}' | sed 's/"//g' | sed 's/,//g' >> /var/plexguide/.emaillist
